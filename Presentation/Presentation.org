...
*** Adjugate: $\text{adj}(\symbb{R}^{n \times n}) \to \symbb{R}^{n \times n}$
:PROPERTIES:
    :BEAMER_env: block
    :BEAMER_col: 0.50
    :BEAMER_act: <3->
...
\begin{talign*}
    A \cdot \text{adj}\left(A\right) & = \det(A) \cdot \symbf{I} \\
    \onslide<4->{& = \begin{bmatrix} \det(A) & 0 \\ 0 & \det(A) \end{bmatrix} \\}
    \onslide<5->{& = \begin{bmatrix} ad - bc & 0 \\ 0 & ad - bc \end{bmatrix} \\}
    \onslide<6->{
...
\begin{algorithm}[H]
    ...
    $i \gets 0$\;
    \While{$i < N$}{
        \ForEach{$M_{ij}$ in $M_i$}{
            $M_{ij} \gets M_{ij} / M_{ii}$\;
        }

        \ForEach{$M_k$ in $M$ where $k \neq i$}{
            \ForEach{$M_{kj}$ in $M_k$}{
                $M_{kj} \gets M_{kj} - M_{ki} \times M_{ij}$\;
            }
        }
