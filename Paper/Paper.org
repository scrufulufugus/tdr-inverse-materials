...
In the process of this research we developed three milestone implementations of matrix inversion. These implementations served as demonstration of our understanding of the problem and as validation of the results of our research. The first implementation, titled ~cpu-inverse~, performs Algorithm [[algo-2]] in a single serial CPU thread. The ~cpu-inverse~ program proves the validity of the inverse algorithm and was useful in troubleshooting race conditions with the parallel programs. The next program, ~inverse~, implements Algorithm [[algo-2]] on-GPU utilizing standard CPU-based kernel synchronization. The ~inverse~ implementation can be likened to the results of Sharma et al. [cite:@sharma:2013] with the optimization provided by DasGupta [cite:@dasgupta:2013]. The final program, ~tdr-inverse~, culminates the results of this research and utilizes the ~Harmonize~ library (See [[#harm]]) to optimize matrix inversion with Thread-Data Remapping.
